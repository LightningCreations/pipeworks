...
class ImageData {
  ...
  public:
    ...
    ImageData(const uint32_t width, const uint32_t height, const uint8_t *data, const void(*deleter)(void*));

    /// \brief Free the image data.
    ///
    /// This calls the deleter function with the parameter set to the stored data.
    void release();
